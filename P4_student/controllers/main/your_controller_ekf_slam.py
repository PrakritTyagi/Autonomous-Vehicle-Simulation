...
class CustomController(BaseController):
    ...
    def update(self, timestep):
        ...
        """
        step = 30
        distNode, NodeIndex = closestNode(X, Y, trajectory) # distance error & nearest waypoint's index
        if step + NodeIndex >= 8203:
            step = 0
        Xn = (trajectory[NodeIndex+step][0]) # X of closest node
        Yn = (trajectory[NodeIndex+step][1]) # X of closest node
        slope_angle = np.arctan2(Yn - Y, Xn - X) # slope of line connection node and vehicle
        ...
        A = sysd.A
        B = sysd.B
        Q = np.diag([0.5,0.5,0.1,0.1])
        R = 20
        S = np.diag([1,1,1,1])
        # Solve LQR Problem
        N = 10
        for i in range(N):
            K = -scipy.linalg.inv(R+B.T@S@B)@B.T@S@A
            S = (A+B*K).T*S*(A+B*K)+Q+K.T*R*K            

        # Now simulate the system
        # Equatinos are from the Documentation
        e1 = distNode 
        e2 = wrapToPi(psi - slope_angle)
        e1dot = ydot + xdot * e2
        e2dot = psidot
        ...
        desired_V = 6
        delV = desired_V - xdot # error in velocity
        
        errorlist.append(delV) 
        curr_err = delV
        prev_err = (delV - errorlist[-2])/delT 
        errorsum = sum(errorlist)
        
        F = kp*curr_err + kd*prev_err + ki*errorsum
